...
const kmsProviders = {
  local: { key: Buffer.from(LOCAL_MASTER_KEY, 'base64') },
};
...
async function run() {
  ...
  try {
    ...
    const keycollection = unencryptedClient
      .db(keyVaultDb)
      .collection(keyVaultColl);
    ...
    try {
      ...
      const current_id = v1().toString();
      await collection
        .insertOne({ name: current_id, encryptedField: 'super secret' })
        .catch((err) => {
          console.error(err.stack);
        });
      console.log('\nDocument inserted.');
      ...
      let doc = await collection.findOne().catch((err) => {
        console.error(err.stack);
      });
      console.log(doc);
    } finally {
